...
handleButtonClicked() {
    ...
    axios.get(
        'https://api.cricapi.com/v1/players?apikey=' + API_KEY + '&offset=0&search=' + searchQuery
      )
      .then((res) => {
        // getData(res.data)]
        ...
        axios.get('https://api.cricapi.com/v1/players_info?apikey=' + API_KEY + '&id=' + playerId)
        .then((res) => {
          ...
          // var output = `<div id="playerName">`;

          var playerImg = data.playerImg;
          ...
          `<div id="upperPart" style="background-color: #F2F2F2">` +
              `<div id="profileImage">` +
                `<img className="border border-dark rounded" src="`+ playerImg +`" />` +
              `</div>` +
